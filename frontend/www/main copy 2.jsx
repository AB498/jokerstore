...
let GeneratorPage = () => {
  ...
  async function generateAndShow() {
    ...
    try {
      ...
      for (let i = 0; i < doc.current.data.images.length; i++) {
        ...
        imageMap[img.input_name] = img.target_index;
        ...
        let fl = (document.querySelector(".form-input." + img.input_name).files[0])
          || await downImage(document.querySelector(".form-input." + img.input_name).parentElement.parentElement.parentElement.querySelector(".upload-image").src)
  ...
  async function overlayImage(imageData, overlayImageData) {
    ...
    await drawOnContext(ctx, overlayImageData, canvas.width / 2, canvas.height / 2);
  ...
  async function fetchAndShow(processId, withoutOverlay = false) {
    ...
    try {
      ...
      let overlayImgData = await getResizedImageData(await drawText("PREVIEW"), resImgData.width / 2, resImgData.width / 2 * 9 / 16);

...
let pollWithCancel = (fn, t, breakTimeout) => {
  let canceller;
...
let elm = new DOMParser().parseFromString(
  `<div
        class="debugcol hover:opacity-75 text-xs items-stretch transition-all duration-300 overflow-auto h-20 w-full dark:text-zinc-50 text-zinc-950/75 bg-zinc-50 dark:bg-zinc-950/75 flex flex-col overflow-auto whitespace-pre-wrap transition-all bottom-0 fixed z-50 backdrop-filter backdrop-blur-sm">
        <div class="flex gap-1 w-full shrink-0 justify-center font-bold bg-zinc-100 dark:bg-zinc-950/75 p-1 sticky top-0 z-10 border-b shadow">
            <div class="debugtitle grow rounded hover:bg-gray-500  bg-zinc-200 dark:bg-zinc-900/75 flex justify-center items-center "
             ...
             if (document.querySelector('.debugcol').classList.contains('h-1/2')) {
               ...
               document.querySelector('.debugcol').classList.add('h-20');
             }else{
               document.querySelector('.debugcol').classList.remove('h-20');
...
window.deb_log = (...args) => {
  ...
  // if (window.localStorage.getItem("debug-auto-open") == "true") document.querySelector(".debugcol").classList.remove("h-20");
};
